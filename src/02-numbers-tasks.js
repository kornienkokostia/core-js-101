 ...
 *   5*x = 0         => 0
 */
const getLinearEquationRoot = (a, b) => (Object.is(b / -a, -0) ? 0 : b / -a);
...
const getAngleBetweenVectors = (x1, y1, x2, y2) => {
  const res = (x1 * x2 + y1 * y2) / (Math.sqrt(x1 * x1 + y1 * y1)
 ...
 *     0     => 0
 */
const getLastDigit = (value) => Number(String(value).split('').pop());
